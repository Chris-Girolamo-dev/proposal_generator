'use client'
// OPFOR motion RangeSlider — adapted from beui.dev/components/motion/range-slider to the
// OPFOR design system (framer-motion + design tokens). Spring-glided fill + thumb that
// follow the pointer and ease between snapped steps; bouncy grab feedback on the thumb.
// Pointer drag + keyboard (arrows / Home / End), reduced-motion aware.
...
// Smooth glide for the thumb/fill — critically damped, no overshoot.
...
export function RangeSlider({
  ...
  // Self-offset the thumb from 0% (flush left) to -100% (flush right) of its own
  // width so it stays fully inside the track at both ends — no clip, no gap.
  ...
  const onPointerDown = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      ...
      setActive(true)
      // Move focus to the thumb so arrow-key adjustment works immediately after a
      // track click (the native input[type=range] gave this for free).
  ...
  return (
    <div
      ...
      className={cn(
        'relative flex h-8 w-full touch-none select-none items-center overflow-hidden rounded-lg bg-canvas',
        disabled ? 'pointer-events-none opacity-50' : 'cursor-grab active:cursor-grabbing',
        className,
      )}
    >
      {/* fill — runs from the left edge to the thumb */}
      <motion.div className="absolute inset-y-0 left-0 bg-fg/15" style={{ width: left }} />
      ...
      <div className="pointer-events-none absolute inset-x-2 inset-y-0">
        {ticks.map((t) => {
          ...
          return (
            <span
              key={t}
              className="absolute top-1/2 size-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-fg/25"
              style={{ left: `${tp}%` }}
            />
          )
        })}
      ...
      <motion.div
        ...
        className="absolute top-1/2 h-5 w-1.5 rounded-sm bg-fg shadow-sm outline-none ring-fg/30 focus-visible:ring-4"
