      ...
      <div className="mt-10 grid grid-cols-2 gap-x-8 gap-y-7">
        {phases.map((phase) => (
          <div key={phase.phase} className="no-break pd-shead pd-shead--sm pt-3">
            ...
            <h3 className="mt-2 pd-display text-[18px] font-semibold tracking-[-0.01em] text-[var(--pd-ink)]">
              {phase.label.replace(/\.$/, "")}
            </h3>
            {phase.tagline && <p className="pd-meta mt-1">{phase.tagline.replace(/\.$/, "")}</p>}
            <p className="mt-2.5 max-w-[46ch] text-[12px] leading-[1.55] text-[var(--pd-tag)]">
              {phase.detail}
            </p>
            {phase.whyItMatters && (
              <p className="mt-1.5 max-w-[46ch] text-[12px] leading-[1.55] text-[var(--pd-dim)]">
                {phase.whyItMatters}
              </p>
            )}
